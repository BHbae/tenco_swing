...
public class Bubble extends JLabel implements Moveable {
	...
	private BackgroundBubbleSevice backgroundBubbleSevice;
	...
	private void initData() {
		...
		backgroundBubbleSevice = new BackgroundBubbleSevice(this);
	...
	public void left() {
		...
		for (int i = 0; i < 400; i++) {
			...
			// 만약 왼쪽벽 --> up()
			if (backgroundBubbleSevice.leftWall()) {
				break;
			}
			try {
				Thread.sleep(1);
	...
	public void right() {
		...
		for (int i = 0; i < 400; i++) {
			x++;
			setLocation(x, y);
			if (backgroundBubbleSevice.rightWall()) {
				break;
			}
			try {
				Thread.sleep(1);
	...
	public void up() {
		...
		while (true) {
			y--;
			setLocation(x, y);
			if (backgroundBubbleSevice.topWall()) {
				break;
			}
			try {
				Thread.sleep(1);
